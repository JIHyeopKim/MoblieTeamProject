...
const Q4StrategyC2 = (props) => {

    const C2 = useState("Strategy C Prompt 2\n")
    
    const Answer2 = useState("Ok, so the four bars represent 84 points.\nThen how many points did Elena score?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
            updateDB()
            props.navigation.navigate("Q4StrategyC3")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["4-3-2"] != null){
