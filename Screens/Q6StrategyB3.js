...
const Q6StrategyB3 = (props) => {

    const B3 = useState("Strategy B Prompt 3\n")

    const Answer3 = useState("Ok, so you guessed that the width is [w] and the length is [w+12].\n" 
    ...
    const submitBtn = () => {
        ...
        var keyword1 = 'value:';
        var keyword2 = '[4w+24]';
        var keyword3 = 'If value is 104';
        var keyword4 = 'go to confirmation';
        var keyword5 = 'Otherwise';
        var keyword6 = 'return to Strategy B Prompt 4';
        ...
        if(checkBoolean(string.includes(keyword2))){
          ...
          if(checkBoolean(string.includes(keyword6))){
            a += 20;
          }
        // 학생이 입력한 답변이 100% 일치하면 다음 strategy 화면으로 넘어가도록 함
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q6StrategyB4")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["6-2-3"] != null){
