...
const Q5StrategyB = (props) => {

    const B1 = useState("Strategy B Prompt 1\n")
    
    const Answer1 = useState("Ok, you want to add on sections to 34.5 inches until you use up all the rope.\nFirst, how many inches of rope does Mario have?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q5StrategyB2")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["5-2-1"] != null){
