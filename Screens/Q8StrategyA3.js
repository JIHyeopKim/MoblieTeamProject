...
const Q8StrategyA3 = (props) => {

    const A3 = useState("Strategy A Prompt 3\n")

    const Answer3 = useState("Ok, so you’re guessing the garden would be [x] feet wide.\nHow much fencing would he need to make the two sides that go along the width?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q8StrategyA4")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["8-1-3"] != null){
