...
const Q5StrategyC2 = (props) => {

    const C2 = useState("Strategy C Prompt 2\n")

    const Answer2 = useState("Nice work!\nNow, how many times can Mario cut 8¼-inch sections from the rope before he has no rope left?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q5StrategyABCConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["5-3-2"] != null){
