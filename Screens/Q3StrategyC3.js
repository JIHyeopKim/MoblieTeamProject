...
const Q3StrategyC3 = (props) => {

    const C3 = useState("Strategy C Prompt 3\n")

    const Answer2 = useState("Fantastic! Now let’s find how long each of those shorter rectangles are.")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q3StrategyABCConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["3-3-3"] != null){
