...
const Q3StrategyA = (props) => {

    const A1 = useState("Strategy A\nPrompt 1\n\n")
    
    const Answer1 = useState("Let’s subtract the extra fabric.\nHow much did Jennifer use for 6 curtains?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q3StrategyA2")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["3-1-1"] != null){
