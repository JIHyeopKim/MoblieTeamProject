...
const Q2StrategyC = (props) => {

    const C1 = useState("Strategy C\nPrompt 1\n")
    
    const Answer1 = useState("Let’s subtract Jen’s miles from Monday through Friday from 22.\nHow many miles does Jen have left to run?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q2StrategyBCConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["2-3"] != null){
