...
const Q4StrategyA3 = (props) => {

    const A3 = useState("Strategy A\nPrompt 4\n")

    const Answer3 = useState("Nice work! Now, what do Elena’s,\nKarla’s and Faye’s scores add up to?")
    ...
    const submitBtn = () => {
        ...
        var keyword1 = 'equivalent to 4x+30';
        var keyword2 = 'If result is 114,';
        var keyword3 = 'go to strategy A confirmation';
        var keyword4 = 'If result is not 114,';
        var keyword5 = 'then return to Prompt 1';
        ...
        if(checkBoolean(string.includes(keyword2))){
          ...
          if(checkBoolean(string.includes(keyword5))){
            a += 20;
          }
        // 학생이 입력한 답변이 100% 일치하면 다음 strategy 화면으로 넘어가도록 함
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q4StrategyA4")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["4-1-3"] != null){
