...
const Q6StrategyC = (props) => {

    const C1 = useState("Strategy C Prompt 1\n")
    
    const Answer1 = useState("Let’s try a diagram to help. The rectangle has two sides with one measure," 
    +" and two sides with a longer measure. How much extra length is on the longer segments?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q6StrategyC2")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["6-3-1"] != null){
