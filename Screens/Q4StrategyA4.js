...
const Q4StrategyA4 = (props) => {

    const A4 = useState("Strategy A\nPrompt 5\n")

    const Answer4 = useState("Nice work! The points for Elena, Karla, and Faye add up to 114,\nso that seems correct! So who scored the most?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q4StrategyABCConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["4-1-4"] != null){
