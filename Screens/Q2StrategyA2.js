...
const Q2StrategyA2 = (props) => {

    const A2 = useState("Strategy A\nPrompt 2\n")

    const Answer2 = useState("OK, if she ran 16 1/8 miles, how many more does she have to run to reach 22 miles?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
            updateDB()
            props.navigation.navigate("Q2StrategyAConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["2-1-2"] != null){
