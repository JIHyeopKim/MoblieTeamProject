...
const Q2StrategyB = (props) => {

    const B1 = useState("Strategy B\nPrompt 1\n")

    const AnswerB = useState("What equation will represent the situation?\nUse the letter “m” as your vairable");
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
            updateDB()
            props.navigation.navigate("Q2StrategyB2")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["2-2-1"] != null){
