    ...
    const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["3-1-1"] != null && row["3-1-2"] != null && row["3-2-1"] != null
                && row["3-2-2"] != null && row["3-3-1"] != null && row["3-3-2"] != null && row["3-3-3"] != null){
                    // 제출한 답변이 있다면 다음 화면으로 넘어가게 됨
                    props.navigation.navigate("Question4")
                }
            })}
        }catch(error){
        ...
        <View style ={styles.mainView}>
            ...
            <View style = {styles.AnswerText}>
                <View style = {styles.AnswerView}>
                    <TouchableOpacity style = {styles.Answerbutton}
                        onPress = { () => {
                            props.navigation.navigate("Q3StrategyA") //클릭시 답변 페이지로 이동
                        }}>
                    <Text style = {styles.AnswerButtonFont}>Strategy A</Text>
                    </TouchableOpacity>
                    <Text style = {styles.AnswerTextFont}>Subtract the extra yards and then figure out how much fabric she used for each curtain</Text>
                </View>
            </View>
            ...
            <View style = {styles.AnswerText}>
                <View style = {styles.AnswerView}>
                    <TouchableOpacity style = {styles.Answerbutton}
                        onPress = { () => {
                            props.navigation.navigate("Q3StrategyB") //클릭시 답변 페이지로 이동
                        }}
                    >
            ...
            <View style = {styles.AnswerText}>
                <View style = {styles.AnswerView}>
                    <TouchableOpacity style = {styles.Answerbutton}
                        onPress = { () => {
                            props.navigation.navigate("Q3StrategyC") //클릭시 답변 페이지로 이동
                        }}
                    >
