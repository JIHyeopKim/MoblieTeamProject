...
const Q8StrategyC3 = (props) => {

    const C3 = useState("Strategy C Prompt 3\n")

    const Answer3 = useState("All right,\nso if you can add 30 feet of fencing before running out,\nthen how wide could the garden be?")
    ...
    const submitBtn = () => {
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q8StrategyBCConfirmation")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["8-3-3"] != null){
