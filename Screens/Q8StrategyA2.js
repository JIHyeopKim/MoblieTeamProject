...
const Q8StrategyA2 = (props) => {

    const A2 = useState("Strategy A Prompt 2\n")

    const Answer2 = useState("Ok, you said he will need 50 feet of fencing for the sides that go along the length of the garden.\nSo, what is the widest garden that you think he could make?")
    ...
    const submitBtn = () => {
        ...
        var keyword1 = 'Answers will';
        var keyword2 = 'vary --';
        var keyword3 = 'use x as variable';
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q8StrategyA3")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["8-1-2"] != null){
