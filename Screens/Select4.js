    ...
    const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["4-1-1"] != null && row["4-1-2"] != null && row["4-1-3"] != null
                && row["4-1-4"] != null && row["4-2-1"] != null && row["4-2-2"] != null 
                && row["4-2-3"] != null && row["4-2-4"] != null && row["4-3-1"] != null && row["4-3-2"] != null
                && row["4-3-3"] != null && row["4-3-4"] != null){
                    // 제출한 답변이 있다면 다음 화면으로 넘어가게 됨
                    props.navigation.navigate("Question5")
                }
            })}
        }catch(error){
