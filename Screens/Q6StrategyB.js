...
const Q6StrategyB = (props) => {

    const B1 = useState("Strategy B Prompt 1\n")
    
    const Answer1 = useState("Ok, you want to guess and check.\nHow long do you think the width is?")
    ...
    const submitBtn = () => {
        ...
        var keyword1 = 'Answers may';
        var keyword2 = 'vary --';
        var keyword3 = 'use w';
        var keyword3_1 = 'use "w"';
        var keyword4 = 'as a variable';
        ...
        if(checkBoolean(string.includes(keyword2))){
          ...
          if(checkBoolean(string.includes(keyword3)) || checkBoolean(string.includes(keyword3_1))){
            a += 25;
          }
        ...
        if(a == 100){
          updateDB()
            props.navigation.navigate("Q6StrategyB2")
            setAnswer("")
          a = 0;
      ...
      const answercheckDB = async ()=>{
        try{
            ...
            {students?.map((row) => {
                ...
                if(stuid == row.addName && row["6-2-1"] != null){
